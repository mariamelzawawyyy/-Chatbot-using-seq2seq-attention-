...
def percentile_max_len(pairs, percentile=95):
    ...
    return int(np.percentile(src_lengths, percentile)), int(np.percentile(tgt_lengths, percentile)) 
...
train_inputs = [
    pad_tensor(t, src_max_len, PAD_IDX)
    for t in train_inputs
]

train_targets = [
    pad_tensor(t, tgt_max_len, PAD_IDX)
