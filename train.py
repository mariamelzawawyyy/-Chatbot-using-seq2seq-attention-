...
# --- Hyperparameters المحسنة ---
embedding_size = 128   # تقليل الحجم لتقليل الحفظ
hidden_size    = 128   # تقليل الحجم لتقليل الحفظ
num_layers     = 2     
attention_dim  = 128
vocab_size     = vocab.n_words
num_epochs     = 20    # زودنا عدد الـ Epochs لأن الـ Early Stopping هيوقفه في الوقت الصح
learning_rate  = 0.001

# --- Initialize ---
encoder = Encoder(vocab_size, embedding_size, hidden_size, num_layers, PAD_IDX, dropout=0.5).to(device)
decoder = Decoder(embedding_size, vocab_size, vocab_size, hidden_size, num_layers, attention_dim, dropout=0.5).to(device)
model = Seq2seq(encoder, decoder).to(device)

# إضافة Label Smoothing
criterion = nn.CrossEntropyLoss(ignore_index=PAD_IDX, label_smoothing=0.1)

# إضافة Weight Decay (L2 Regularization)
optimizer = optim.Adam(model.parameters(), lr=learning_rate, weight_decay=1e-4)
scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=1)

# متغيرات الـ Early Stopping
best_val_loss = float('inf')
patience = 3
counter = 0

print("Starting Anti-Overfitting Training Session...")
...
for epoch in range(1, num_epochs + 1):
    ...
    print(f"Epoch [{epoch}] | TF: {tf_ratio:.2f} | Train: {avg_train_loss:.4f} | Val: {avg_val_loss:.4f} {status}")
    
    if counter >= patience:
        print("🛑 Early Stopping triggered. Model is starting to overfit.")
        break
